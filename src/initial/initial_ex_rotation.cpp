...
bool InitialEXRotation::CalibrationExRotation(
  ...
  for (int i = 1; i <= frame_count; i++) {
    const Quaterniond r1(Rc[i]);
    const Quaterniond r2(Rc_g[i]);

    const double angular_distance = 180 / M_PI * r1.angularDistance(r2);
    const double huber = angular_distance > 5.0 ? 5.0 / angular_distance : 1.0;
    ++sum_ok;

    double w = Quaterniond(Rc[i]).w();
    Vector3d q = Quaterniond(Rc[i]).vec();

    Matrix4d L = Matrix4d::Zero();
...
Matrix3d InitialEXRotation::solveRelativeR(
  ...
  if (corres.size() >= 9) {
    ...
    const double ratio1 = max(testTriangulation(ll, rr, R1, t1),
                              testTriangulation(ll, rr, R1, t2));
    const double ratio2 = max(testTriangulation(ll, rr, R2, t1),
...
double InitialEXRotation::testTriangulation(const vector<cv::Point2f> &l,
                                            ...
                                            cv::Mat_<double> R,
                                            cv::Mat_<double> t) {
  const cv::Matx34f P1(R(0, 0), R(0, 1), R(0, 2), t(0), R(1, 0), R(1, 1),
  ...
  for (int i = 0; i < pointcloud.cols; i++) {
    const double normal_factor = pointcloud.col(i).at<float>(3);

    const cv::Mat_<double> p_3d_l =
        cv::Mat(P) * (pointcloud.col(i) / normal_factor);
    const cv::Mat_<double> p_3d_r =
