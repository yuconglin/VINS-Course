...
bool FeatureManager::addFeatureCheckParallax(
  ...
  if (parallax_num == 0) {
    ...
    // parallax_num); ROS_DEBUG("current parallax: %lf", parallax_sum /
...
vector<pair<Vector3d, Vector3d>> FeatureManager::getCorresponding(
  ...
  for (auto &it : feature) {
    if (it.start_frame <= frame_count_l && it.endFrame() >= frame_count_r) {
      const int idx_l = frame_count_l - it.start_frame;
      const int idx_r = frame_count_r - it.start_frame;

      const Vector3d &a = it.feature_per_frame[idx_l].point;
      const Vector3d &b = it.feature_per_frame[idx_r].point;
...
// Removes feature points that fail in depth estimation.
void FeatureManager::removeFailures() {
  for (auto it = feature.begin(); it != feature.end(); ++it) {
    if (it->solve_flag == 2) {
      it = prev(feature.erase(it));
    }
  }
}
  ...
  for (auto &it_per_id : feature) {
    ...
    const Eigen::Vector4d svd_V =
        Eigen::JacobiSVD<Eigen::MatrixXd>(svd_A, Eigen::ComputeThinV)
            .matrixV()
            .rightCols<1>();
    const double svd_method = svd_V[2] / svd_V[3];
...
void FeatureManager::removeOutlier() { return; }

// Remove the oldest element and re-calculate the depth.
  ...
  for (auto it = feature.begin(); it != feature.end(); ++it) {
    if (it->start_frame != 0) {
      ...
      if (it->feature_per_frame.size() < 2) {
        // "erase" will return the element after the removed one or the end().
        // Therefore we need it to point to the previous one to deal with
        // "++it".
        it = prev(feature.erase(it));
        continue;
      } else {
        const Eigen::Vector3d &uv_i = it->feature_per_frame[0].point;
        const Eigen::Vector3d pts_i = uv_i * it->estimated_depth;
        const Eigen::Vector3d w_pts_i = marg_R * pts_i + marg_P;
        const Eigen::Vector3d pts_j = new_R.transpose() * (w_pts_i - new_P);

        const double dep_j = pts_j(2);
        if (dep_j > 0) {
...
// Only removes the oldest element.
...
// Only removes the second newest element: frame_count - 1.
void FeatureManager::removeFront(int frame_count) {
  for (auto it = feature.begin(); it != feature.end(); ++it) {
    if (it->start_frame == frame_count) {
      ...
      const int j = WINDOW_SIZE - 1 - it->start_frame;
...
double FeatureManager::compensatedParallax2(const FeaturePerId &it_per_id,
  ...
  const double du_comp = u_i_comp - u_j;
  const double dv_comp = v_i_comp - v_j;

  return sqrt(min(du * du + dv * dv, du_comp * du_comp + dv_comp * dv_comp));
}