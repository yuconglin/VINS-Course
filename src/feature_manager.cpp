...
// Add features while checking parallax.
bool FeatureManager::addFeatureCheckParallax(
    int frame_count,
    const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &images,
    double td) {
...
void FeatureManager::triangulate(Vector3d Ps[], Vector3d tic[],
  ...
  for (auto &it_per_id : feature) {
    ...
    for (const auto &it_per_frame : it_per_id.feature_per_frame) {
      ...
      const Eigen::Vector3d f = it_per_frame.point.normalized();
