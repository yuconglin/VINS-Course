...
bool Estimator::initialStructure() {
  ...
  // check imu observibility
  {
    ...
    Vector3d aver_g;
    aver_g = sum_g * 1.0 / (all_image_frame.size() - 1);
    double var = 0;
    for (frame_it = all_image_frame.begin(), frame_it++;
         frame_it != all_image_frame.end(); frame_it++) {
      double dt = frame_it->second.pre_integration->sum_dt;
  ...
  // Prepares the sfm feature points.
  for (const auto &it_per_id : f_manager.feature) {
    int imu_j = it_per_id.start_frame - 1;

    SFMFeature tmp_feature{.state = false, .id = it_per_id.feature_id};
  ...
  // Updates the pose for every frame by PnP with intialized 3d points from SFM
  // and corresponding 2d points from images.
  ...
  for (int i = 0; frame_it != all_image_frame.end(); frame_it++) {
    ...
    for (auto &[feature_id, points] : frame_it->second.points) {
      for (const auto &i_p : points) {
...
// Finds the first frame l and its intial relative {R, T} via fundmental matrix
// method.
...
void Estimator::solveOdometry() {
  ...
  if (solver_flag == NON_LINEAR) {
    TicToc t_tri;
    // Computes the inversed depth for each feature point.
