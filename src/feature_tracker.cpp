...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time) {
  ...
  if (PUB_THIS_FRAME) {
    ...
    if (n_max_cnt > 0) {
      if (mask.empty()) LOG(INFO) << "mask is empty " << endl;
      if (mask.type() != CV_8UC1) LOG(INFO) << "mask type wrong " << endl;
      if (mask.size() != forw_img.size()) LOG(INFO) << "wrong size " << endl;
