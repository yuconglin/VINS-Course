...
void HuberLoss::Compute(double e, Eigen::Vector3d& rho) const {
  ...
  if (e <= dsqr) {  // inlier
    ...
    const double sqrte = sqrt(e);  // absolut value of the error
    rho[0] =
...
void CauchyLoss::Compute(double err2, Eigen::Vector3d& rho) const {
  const double dsqr = delta_ * delta_;       // c^2
  const double dsqrReci = 1. / dsqr;         // 1/c^2
  const double aux = dsqrReci * err2 + 1.0;  // 1 + e^2/c^2
