...
bool Problem::AddVertex(std::shared_ptr<Vertex> vertex) {
  if (verticies_.find(vertex->Id()) != verticies_.end()) {
    ...
    verticies_.insert(
        pair<unsigned long, shared_ptr<Vertex>>(vertex->Id(), vertex));
  }
...
void Problem::AddOrderingSLAM(std::shared_ptr<myslam::backend::Vertex> v) {
  if (IsPoseVertex(v)) {
    ...
    idx_landmark_vertices_.insert(
        pair<ulong, std::shared_ptr<Vertex>>(v->Id(), v));
  }
}
...
bool Problem::Solve(int iterations) {
  ...
  while (!stop && (iter < iterations)) {
    ...
    while (!oneStepSuccess && false_cnt < 10) {
      ...
      // TODO:: 退出条件还是有问题,
      // 好多次误差都没变化了，还在迭代计算，应该搞一个误差不变了就中止
      //            if ( false_cnt > 10)
      //            {
      //                stop = true;
      //                break;
...
bool Problem::Marginalize(
    const std::vector<std::shared_ptr<Vertex>> margVertexs, int pose_dim) {
  SetOrdering();
  /// 找到需要 marg 的 edge, margVertexs[0] is frame, its edge contained
  /// pre-intergration
