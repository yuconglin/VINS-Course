...
void EdgeReprojection::ComputeResidual() {
  //   LOG(INFO) << pts_i_.transpose() <<" "<<pts_j_.transpose()  <<std::endl;
...
// void EdgeReprojection::SetTranslationImuFromCamera(Eigen::Quaterniond &qic_,
// Vec3 &tic_) {
//    qic = qic_;
//    tic = tic_;
//}
...
void EdgeReprojection::ComputeJacobians() {
  ...
  ///------------- check jacobians -----------------
  //    {
  //       LOG(INFO) << jacobians_[0] <<std::endl;
  ...
  //       LOG(INFO) <<"num jacobian: "<<  (tmp_residual - residual_) / eps
  //       <<std::endl;
  //    }
}
